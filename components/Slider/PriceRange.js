...
const PriceRange = React.createClass({
  ...
  onSelect: function () {
    ...
    var props = this.props;
    var prev = this._prevState;

    props.onSelect(state.leftValue, state.rightValue);

    if (!prev) return;

    if (prev.leftValue !== state.leftValue ||
        prev.rightValue !== state.rightValue) {
      props.onChange(state.leftValue, state.rightValue)
    }
  },
